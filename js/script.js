...
/* ---------------- Lightbox (zoomable photos) ----------------
   Defined at top level (not inside DOMContentLoaded) so that
   window.LightboxUtil already exists by the time gallery-loader.js's
   own DOMContentLoaded handler runs and checks for it. */
window.LightboxUtil = (function () {
  let modal, imgEl, captionEl, lastFocused;
  ...
  function bind(el) {
    ...
    el.addEventListener("click", () => {
      open(el.dataset.full || el.src, el.dataset.caption || el.alt || "");
    });
  }
...
function initLightbox() {
  // Bind any zoomable images already present at load time. Images added
  // later (e.g. by gallery-loader.js) are bound directly by that script
  // via window.LightboxUtil.bind().
  document.querySelectorAll("img.zoomable").forEach((el) => {
    window.LightboxUtil.bind(el);
  });
}
